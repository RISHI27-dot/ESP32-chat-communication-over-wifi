...
{
    espnow_event_t evt;// ## creats an event handle ## 
    espnow_event_send_cb_t *send_cb = &evt.info.send_cb;// ## setting the pointer to the send callback field of the event
    ...
    evt.id = ESPNOW_SEND_CB;// ## weather thew event is send or recive
    /*now the mac address is set which is recived as the parameter of the fucntion
      memcpy copies the given number of bytes from the source to the destination*/
    memcpy(send_cb->mac_addr, mac_addr, ESP_NOW_ETH_ALEN);
    /* set the status recived as the parameter */
    send_cb->status = status;
    /* sending the event to the queue*/
...
void espnow_data_prepare(espnow_send_param_t *send_param)
{
    ...
    ESP_LOGI(TAG, "entering espnow_data_prepare");
    int r;
    ...
    ESP_LOGI(TAG, "exiting espnow_data_prepare");
    
}

static void espnow_task(void *pvParameter)
{
    ESP_LOGI(TAG, "entering espnow_task");
    espnow_event_t evt;
    uint8_t recv_state = 0;// if the recived data is broadcast or not
    uint16_t recv_seq = 0;//sequence number of esp data 
    int recv_magic = 0;//to deternime to which device to send unicast data
    ...
    ESP_LOGI(TAG, "exiting espnow_task");
}

static esp_err_t espnow_init(void)
{
    ESP_LOGI(TAG, "entering espnow_init");
    ...
    ESP_ERROR_CHECK( esp_now_register_send_cb(espnow_send_cb) );//register callback
    ESP_ERROR_CHECK( esp_now_register_recv_cb(espnow_recv_cb) );//register callback
    ...
    memset(peer, 0, sizeof(esp_now_peer_info_t));//sets the members to zero
    peer->channel = CONFIG_ESPNOW_CHANNEL;
    peer->ifidx = ESPNOW_WIFI_IF;
    peer->encrypt = false;
    ...
    memset(send_param, 0, sizeof(espnow_send_param_t));//sets all members to zero
    if (send_param == NULL) {
    ...
    return ESP_OK;
    ESP_LOGI(TAG, "exiting espnow_init");
}
