...
static void register_espnow_setup(void);
...
static void register_espnow_setup(void)
{
    const esp_console_cmd_t espnow_setup_cmd = {
        .command = "setup",
        .help = "setup esp now for reciving and sending",
        .hint = NULL,
        .func = &espnow_setup,
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&espnow_setup_cmd));
}
